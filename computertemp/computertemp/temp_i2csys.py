...
class TempFuncs:
	...
	def __init__(self):
		...
		self.thermal_path = '/sys/bus/i2c/devices/'
	...
	def get_sensor_name(self): return 'Kernel i2c sensors'
	...
	def get_zones(self): 
		if self.temp_support():
			...
			for i in xrange(len(self.i2csensors)):
				lista = (dircache.listdir(self.thermal_path+'/'+self.i2csensors[i]))
				for j in xrange(len(lista)):
					if re.search('temp[0-9]*_input', lista[j]) != None:
						zones.append(self.i2csensors[i]+'/'+lista[j])
